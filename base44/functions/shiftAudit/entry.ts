...
Deno.serve(async (req) => {
  try {
    ...
    for (const abs of allAbsences) {
      ...
      // Una ausencia auto-detectada que fue APROBADA por RRHH se trata como formal:
      // el empleado ya está gestionado y no debe volver a aparecer en detección diaria.
      const isApprovedByRRHH = isAutoGenerated && abs.estado_aprobacion === 'Aprobada';

      if (isAutoGenerated && !isApprovedByRRHH) {
        // Ausencia auto pendiente (sin revisar aún)
        ...
        // Ausencia formal (manual/RRHH) O ausencia auto ya aprobada por RRHH
        // En ambos casos, el empleado está gestionado y no debe re-detectarse
