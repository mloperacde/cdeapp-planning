...
import {
  ...
  BarChart3, ClipboardCheck, Factory, Briefcase, Shield, Clock, FileText, Target
} from "lucide-react";
  ...
  return (
    <div className="h-full flex flex-col p-3 md:p-6 gap-4 md:gap-6 bg-slate-50 dark:bg-slate-950">
      ...
      <div className="flex flex-col gap-4 md:gap-6">
        ...
        <section aria-label="Accesos rápidos">
          ...
          <div className="flex gap-3 overflow-x-auto pb-1 md:grid md:grid-cols-5 md:overflow-visible scrollbar-hide">
            {quickActions.map(action => {
              ...
              return (
                <Link key={action.title} to={createPageUrl(action.url)} className="flex-shrink-0 w-36 md:w-auto" aria-label={action.title}>
                  <Card className="h-full hover:shadow-xl transition-all duration-300 border-0 bg-white dark:bg-slate-800 cursor-pointer group active:scale-95">
                    <CardContent className="p-4 md:p-6">
                      <div className={`w-10 h-10 md:w-12 md:h-12 rounded-lg bg-gradient-to-br ${COLOR_CLASSES[action.color]} flex items-center justify-center mb-3 group-hover:scale-110 transition-transform duration-300 shadow-lg`} aria-hidden="true">
        ...
        {/* Tablones – Shift swap + Announcements */}
