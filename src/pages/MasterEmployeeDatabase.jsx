...
export default function MasterEmployeeDatabasePage() {
  ...
  const stats = useMemo(() => {
    if (!masterEmployees) return { total: 0, active: 0, absent: 0, departments: 0 };
    return {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col p-2 gap-2 bg-slate-50 dark:bg-slate-950 overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 shrink-0 bg-white dark:bg-slate-900 p-2 px-3 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
          ...
          <Button
            ...
            className={`h-7 text-xs px-2 ${showFilters ? 'bg-slate-100 dark:bg-slate-800' : ''}`}
          >
      ...
      <Card className="flex-1 flex flex-col min-h-0 border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden bg-white dark:bg-slate-900">
        {showFilters && (
          <div className="p-2 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 shrink-0 animate-in slide-in-from-top-2 duration-200">
            <AdvancedSearch
