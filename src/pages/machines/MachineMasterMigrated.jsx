...
import { processRecoveryService } from '@/services/processRecoveryService';
...
export default function MachineMasterPage() {
  ...
  const { data: machines = EMPTY_ARRAY, isLoading } = useQuery({
    queryKey: ['machines'],
    queryFn: () => machineService.list('orden'),
    staleTime: 5 * 60 * 1000,
    initialData: EMPTY_ARRAY,
  });

  const createMutation = useMutation({
    mutationFn: (data) => machineService.create(data),
    onSuccess: () => {
  ...
  const handleRecoverProcesses = async () => {
    try {
      ...
      const result = await processRecoveryService.recoverProcesses();

          toast.dismiss();
    toast.success(`
      Recuperados ${result.totalProcesses} procesos.
      ${result.assignedProcesses} están asignados a máquinas.
    `);
    
    // Mostrar alert con los datos
    alert(`
      PROCESOS RECUPERADOS (${result.totalProcesses})
      =============================
      ${result.recoveredProcesses.map(p => `
        • ${p.nombre} (${p.codigo})
