...
import {
  ...
  ShieldCheck, ShieldAlert, Clock, CalendarCheck
} from "lucide-react";
    ...
    <div className="h-full flex flex-col p-3 md:p-6 gap-4 bg-slate-50 dark:bg-slate-950 overflow-y-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 shrink-0 bg-white dark:bg-slate-900 p-2 px-3 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-bold text-slate-900 dark:text-slate-100 leading-tight">Sincronización Cuco360</h1>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 hidden sm:block">
              Verifica y sincroniza empleados entre la BD Maestra y Cuco360
            </p>
          </div>
        </div>
      ...
      <div className="space-y-4">
        ...
        {lsr && (
          <Card className={`border-2 ${lsr.success ? "border-green-300 bg-green-50 dark:bg-green-950/20" : lsr.integrity?.employees_failed > 0 ? "border-amber-300 bg-amber-50 dark:bg-amber-950/20" : "border-red-300 bg-red-50 dark:bg-red-950/20"}`}>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm flex items-center gap-2">
                ...
                <span>Resultado del Último Sync</span>
                {lsr.duration_seconds && (
                  <span className="flex items-center gap-1 text-xs font-normal text-slate-500 ml-auto">
                    <Clock className="w-3 h-3" />{lsr.duration_seconds}s
                  </span>
                )}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <p className={`text-sm font-medium ${lsr.success ? "text-green-700" : "text-amber-700"}`}>{lsr.message}</p>

              {/* KPIs del sync */}
              {lsr.integrity && (
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-white dark:bg-slate-800 rounded-lg p-2 text-center border">
                    <p className="text-lg font-bold text-slate-800 dark:text-slate-100">{lsr.integrity.employees_total}</p>
                    <p className="text-[10px] text-slate-500">Empleados totales</p>
                  </div>
                  <div className="bg-white dark:bg-slate-800 rounded-lg p-2 text-center border border-green-200">
                    <p className="text-lg font-bold text-green-600">{lsr.integrity.employees_ok}</p>
                    <p className="text-[10px] text-slate-500">Sincronizados OK</p>
                  </div>
                  <div className={`bg-white dark:bg-slate-800 rounded-lg p-2 text-center border ${lsr.integrity.employees_failed > 0 ? "border-red-200" : "border-green-200"}`}>
                    <p className={`text-lg font-bold ${lsr.integrity.employees_failed > 0 ? "text-red-600" : "text-green-600"}`}>{lsr.integrity.employees_failed}</p>
                    <p className="text-[10px] text-slate-500">Con errores</p>
                  </div>
                </div>
              )}

              {/* Verificación de integridad por día */}
              {lsr.integrity?.days?.length > 0 && (
                <div className="space-y-1">
                  <p className="text-xs font-semibold text-slate-600">Verificación de integridad por día:</p>
                  {lsr.integrity.days.map(d => (
                    <div key={d.date} className={`flex items-center justify-between text-xs px-3 py-1.5 rounded ${d.match ? "bg-green-100 dark:bg-green-900/20" : "bg-red-100 dark:bg-red-900/20"}`}>
                      <span className="font-medium">{d.date}</span>
                      <div className="flex items-center gap-2">
                        <span className="text-slate-600">{d.actual_in_db} / {d.expected} registros</span>
                        {d.match ? (
                          <CheckCircle2 className="w-3.5 h-3.5 text-green-600" />
                        ) : (
                          <span className="text-red-600 font-semibold">⚠ Faltan {d.missing}</span>
                        )}
                      </div>
                    </div>
                  ))}
              ...
              {/* Empleados fallidos */}
              {lsr.integrity?.failed_list?.length > 0 && (
                <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 rounded-lg p-3">
                  <p className="text-xs font-semibold text-red-700 mb-1.5">Empleados que fallaron tras 3 intentos:</p>
                  <div className="space-y-1">
                    {lsr.integrity.failed_list.map(f => (
                      <div key={f.code} className="flex items-start gap-2 text-xs">
                        <AlertCircle className="w-3.5 h-3.5 text-red-500 mt-0.5 shrink-0" />
                        <div>
                          <span className="font-mono font-semibold text-red-700">Cód. {f.code}</span>
                  ...
                  {cr.discrepancies.only_in_cuco.map(e => (
                    <tr key={e.codigo} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-2.5 text-right">
                        <Badge variant="outline" className="text-red-600 border-red-200 text-xs">Sin registro en BD Maestra</Badge>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-4 text-center">
                ...
                <p className="text-xs text-slate-500 mt-1">Crear en BD Maestra → sincroniza a Cuco360 automáticamente</p>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-4 text-center">
                <RefreshCw className="w-8 h-8 text-blue-600 mx-auto mb-2" />
                <p className="font-semibold text-sm">Actualización</p>
                <p className="text-xs text-slate-500 mt-1">Modificar datos en BD Maestra → sincroniza cambios a Cuco360</p>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-4 text-center">
                <UserMinus className="w-8 h-8 text-red-600 mx-auto mb-2" />
                <p className="font-semibold text-sm">Baja de Empleado</p>
                <p className="text-xs text-slate-500 mt-1">Cambiar estado a "Baja" en BD Maestra → da de baja en Cuco360</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <td className="px-4 py-2.5">
        {emp.tiene_credenciales !== undefined ? (
          emp.tiene_credenciales ? (
            ...
            <Badge variant="outline" className="text-red-500 border-red-200 text-[10px]">Sin PIN ni tarjeta</Badge>
          )
        ) : null}
      </td>
      <td className="px-4 py-2.5">
        {emp.posible_match_cuco ? (
          <div className="flex flex-col gap-1">
            <div className="flex items-start gap-1 text-amber-700 text-xs">
              ...
              <span>Mismo nombre en Cuco con código <strong>{emp.posible_match_cuco.cod_int}</strong></span>
            </div>
            <Button
              size="sm" variant="outline"
              className="text-[10px] h-6 border-amber-400 text-amber-700 hover:bg-amber-50 px-2"
              onClick={() => onFixCode(emp.id, emp.posible_match_cuco.cod_int, emp.nombre)}
            >
