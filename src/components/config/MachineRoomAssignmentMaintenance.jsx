...
import { Settings, Cog, MapPin, Search, CheckCircle2, AlertCircle } from "lucide-react";
...
/**
 * Versión independiente de MachineRoomAssignment para Mantenimiento.
 * Guarda las asignaciones dentro del config del departamento (config.machine_assignments)
 * en lugar de en MachineMasterDatabase, para que sean independientes de Fabricación.
 */
export default function MachineRoomAssignmentMaintenance({ config, setConfig }) {
  ...
  const { data: machines = [], isLoading } = useQuery({
    queryKey: ['machines_maintenance_assignment'],
    ...
    <div className="space-y-6 p-2">
      <div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
          ...
          Asignación de Máquinas a Zonas de Mantenimiento
        </h2>
        <p className="text-sm text-slate-500 mt-1">
          Asigna cada máquina a una zona y sala de mantenimiento (independiente de Fabricación)
        </p>
      </div>
      ...
      {config.areas?.length === 0 || !config.areas ? (
        <div className="p-6 text-center text-slate-400 border-2 border-dashed rounded-lg">
          ...
          <p>Primero configura las Áreas/Salas de Mantenimiento en la pestaña correspondiente</p>
        </div>
      ) : (
        <>
                        ...
                        return (
                          <div key={machine.id} className="grid grid-cols-12 gap-4 p-3 items-center hover:bg-slate-50 transition-colors">
                            ...
                            <div className="col-span-2">
                              {asgn.area_name ? (
                                <Badge variant="outline" className="bg-orange-50 text-orange-700 border-orange-200">{asgn.area_name}</Badge>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Asignar Máquina a Zona de Mantenimiento</DialogTitle>
            <DialogDescription>
          ...
          {selectedMachine && (
            <div className="space-y-4 py-4">
              <div className="p-3 bg-slate-50 rounded-lg border">
                ...
                <div className="flex gap-2 text-sm">
                  {selectedMachine.currentAreaName ? (
                    <>
                      <Badge className="bg-orange-600">{selectedMachine.currentAreaName}</Badge>
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">Zona de Mantenimiento</label>
