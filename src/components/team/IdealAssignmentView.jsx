  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h2 className="text-xl font-bold text-slate-900">Asignaciones de Máquinas - Configuración de Equipos Ideales</h2>
            <p className="text-slate-500 text-sm">Define la estructura óptima para cada máquina por turno.</p>
        </div>
        
        <div className="flex items-center gap-2 bg-white p-2 rounded-lg shadow-sm border">
            <Label className="whitespace-nowrap font-medium px-2">Equipo de Turno:</Label>
      ...
      <div className="h-[calc(100vh-220px)] overflow-y-auto pr-2 pb-10">
        <div className="grid gap-6">
            {machines.map(machine => {
                const assignment = assignments[machine.id] || {};
                
                // Prepare candidates list filtering exclusions
                const getFilteredCandidates = (role, currentValue) => {
                    const candidates = getCandidatesForDropdown(machine.id, role, assignment);
                    // Filter out employees assigned to OTHER slots in this machine
                    return candidates.filter(c => {
                        // If it is the currently selected value for this dropdown, KEEP IT
                        if (c.id === currentValue) return true;
                        
                        // Check if used in other slots
                        const usedInResp = assignment.responsable_linea === c.id;
                        const usedInSeg = assignment.segunda_linea === c.id;
                        const usedInOps = [1,2,3,4,5].some(i => assignment[`operador_${i}`] === c.id);
                        
                        // If role is RESPONSABLE, exclude if used in SEG or OPS
                        if (role === "RESPONSABLE") return !usedInSeg && !usedInOps;
                        // If role is SEGUNDA, exclude if used in RESP or OPS
                        if (role === "SEGUNDA") return !usedInResp && !usedInOps;
                        // If role is OPERARIO, exclude if used in RESP or SEG or OTHER OPS slots
                        // Note: usedInOps includes 'currentValue' if it was assigned to an operator slot.
                        // But we already returned true if c.id === currentValue (which covers the specific op slot).
                        // So here usedInOps means "used in ANY op slot".
                        // If we are configuring operator_1, and user is in operator_2, we should exclude.
                        if (role === "OPERARIO") return !usedInResp && !usedInSeg && !usedInOps;
                    ...
                    <Card key={machine.id} className="overflow-hidden border-l-4 border-l-blue-500 shadow-sm hover:shadow-md transition-shadow">
                        <CardHeader className="bg-slate-50 border-b pb-4">
                            <div className="flex justify-between items-center">
                                <div>
                                    ...
                                    <div className="flex gap-4 text-sm text-slate-500 mt-1 pl-7">
                        ...
                        <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div className="space-y-3 p-3 rounded-lg bg-blue-50/50 border border-blue-100">
                                ...
                                <EmployeeSelect
                                    employees={getFilteredCandidates("RESPONSABLE", assignment.responsable_linea)}
                            ...
                            <div className="space-y-3 p-3 rounded-lg bg-indigo-50/50 border border-indigo-100">
                                <Label className="flex items-center gap-2 text-indigo-800 font-semibold">
                                    <User className="w-4 h-4" /> Segunda de Línea
                                </Label>
                                <EmployeeSelect
                                    employees={getFilteredCandidates("SEGUNDA", assignment.segunda_linea)}
                            ...
                            {/* Operarios */}
                            <div className="space-y-3 lg:col-span-1 md:col-span-2 p-3 rounded-lg bg-slate-50/50 border border-slate-100">
                                ...
                                <div className="space-y-2">
                                    {[1, 2, 3, 4, 5].map(num => (
                                        <div key={num} className="flex gap-2 items-center">
                                            <Badge variant="outline" className="w-6 h-6 flex items-center justify-center shrink-0 bg-white text-slate-500 text-[10px]">
                                                {num}
                                            </Badge>
                                            <div className="flex-1">
                                                <EmployeeSelect
                                                    employees={getFilteredCandidates("OPERARIO", assignment[`operador_${num}`])}
