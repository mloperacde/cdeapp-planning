...
import { Plus, Save, Trash2, Edit, Wrench, Clock, ArrowRight } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
...
export default function MaintenancePlanningTab({ selectedDate, selectedTeam, selectedShift, teams = [], teamSchedules = [] }) {
  ...
  const { data: plannings, isLoading } = useQuery({
    queryKey: ['dailyMaintenancePlannings', selectedDate, selectedTeam],
    queryFn: () => base44.entities.DailyMaintenancePlanning.list(),
    initialData: [],
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => {
      ...
      if (editingPlanning?.id) {
        return base44.entities.DailyMaintenancePlanning.update(editingPlanning.id, finalData);
      }
      return base44.entities.DailyMaintenancePlanning.create(finalData);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['dailyMaintenancePlannings'] });
      handleClose();
    },
  });
  ...
  const handleEdit = (planning) => {
    ...
    setFormData({
      ...
      area_responsabilidad: planning.area_responsabilidad || "",
      prioridad: planning.prioridad || "Media",
      maquinas_asignadas: planning.maquinas_asignadas || [],
  ...
  // Filter Employees
  const maintenanceEmployees = employees.filter(emp => 
    emp.departamento === "MANTENIMIENTO" && 
  ...
  const employees14h = useMemo(() => {
    if (!tardeTeam) return [];
    return maintenanceEmployees.filter(emp => emp.equipo === tardeTeam.team_name);
  }, [maintenanceEmployees, tardeTeam]);

  const filteredPlannings = plannings.filter(
    p => p.fecha === selectedDate && p.team_key === selectedTeam
  );

  // Plannings specifically for 14h slot (heuristic: starts at 14:00)
  const plannings14h = filteredPlannings.filter(p => p.hora_inicio === '14:00');
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Module 14:00 - 15:00 */}
      <Card className="shadow-lg border-0 bg-gradient-to-r from-indigo-50 to-blue-50 border-l-4 border-l-indigo-500">
            ...
            {employees14h.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {employees14h.map(emp => {
                        ...
                        return (
                            <div key={emp.id} className="flex flex-col gap-2 p-3 bg-white rounded-lg border border-indigo-100 shadow-sm hover:shadow-md transition-shadow">
                                ...
                                {task ? (
                                    ...
                                    <Button 
                                        variant="ghost" 
                                        size="sm" 
                                        className="w-full text-xs h-7 border border-dashed border-slate-300 text-slate-500 hover:text-indigo-600 hover:border-indigo-300 hover:bg-indigo-50"
                                ...
                                {task && (
                                     <Button 
                                        variant="link" 
                                        size="sm" 
                                        className="text-[10px] h-auto p-0 self-end text-indigo-600"
        ...
        <CardHeader className="border-b border-orange-100">
          <div className="flex justify-between items-center">
            ...
            <Button
              onClick={() => {
                  setEditingPlanning(null);
                  setFormData({ ...formData, employee_id: "", hora_inicio: "", hora_fin: "" }); // Reset
                  setShowForm(true);
              }}
                ...
                <TableBody>
                  {filteredPlannings.map((planning) => (
                    <TableRow key={planning.id}>
                      ...
                      <TableCell>{planning.area_responsabilidad || '-'}</TableCell>
                      <TableCell>
                        <Badge className={
                      ...
                      <TableCell>
                        {planning.hora_inicio && planning.hora_fin 
                          ? `${planning.hora_inicio} - ${planning.hora_fin}` 
                          : '-'}
                      </TableCell>
                      <TableCell>
      ...
      {showForm && (
        <Card className="shadow-xl border-2 border-orange-300">
          <CardHeader className="bg-orange-50 border-b border-orange-200">
            <CardTitle>
              {editingPlanning ? 'Editar Asignación' : 'Nueva Asignación de Mantenimiento'}
            </CardTitle>
          </CardHeader>
          <CardContent className="p-6">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, area_responsabilidad: e.target.value })}
                    placeholder="Ej: Zona A, Línea 1-5"
                  />
                </div>
