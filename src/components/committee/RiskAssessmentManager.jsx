          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-amber-900">{inProgressRisks}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-900">{completedRisks}</p>
              </div>
              ...
              {assessments.map((assessment) => (
                <div key={assessment.id} className="border border-slate-200 rounded-lg p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="font-bold text-slate-900">{assessment.titulo}</h3>
                      <p className="text-sm text-slate-600">
                        {assessment.departamento} {assessment.puesto && `- ${assessment.puesto}`}
                      </p>
                    </div>
                    <Badge className={
                      assessment.estado_general === "Completada" ? "bg-green-600" :
                      assessment.estado_general === "En Proceso" ? "bg-blue-600" :
                      "bg-amber-600"
                    }>
                    ...
                    <div className="space-y-2">
                      {assessment.riesgos_identificados.slice(0, 3).map((riesgo, idx) => (
                        <div key={idx} className="text-sm p-2 bg-slate-50 rounded">
                          <div className="flex items-center justify-between">
                            <span className="font-medium">{riesgo.descripcion}</span>
                            <Badge variant="outline" className={
                              riesgo.nivel_riesgo === "Intolerable" || riesgo.nivel_riesgo === "Importante" ? "border-red-600 text-red-600" :
                              riesgo.nivel_riesgo === "Moderado" ? "border-amber-600 text-amber-600" :
