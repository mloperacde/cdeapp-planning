...
import { UserX, Plus, Edit, Trash2, Search, CheckCircle2, AlertCircle, Clock, FileText, Sparkles, RefreshCw, Bot, ShieldAlert } from "lucide-react";
...
export default function UnifiedAbsenceManager(props) {
  ...
  // Consolidado de ausencias activas — SOLO Pendiente y Aprobada (nunca Cancelada ni Rechazada)
  ...
  // Detectar si una ausencia fue auto-generada por el sistema (solo pendientes sin clasificar)
  const isAutoGenerated = isAutoAbsence;
  ...
  const availabilityStats = useMemo(() => {
    ...
    // Contar empleados únicos ausentes (formales + auto-generadas)
    // Los empleados con ausencia auto-generada se cuentan solo una vez aunque tengan varias
    ...
    // Separar formales vs auto-generadas para métricas
    const formalAbsences = relevantAbsences.filter(a => !isAutoGenerated(a));
    const autoAbsences = relevantAbsences.filter(a => isAutoGenerated(a));
    const uniqueFormalAbsent = new Set(formalAbsences.map(a => a.employee_id)).size;
    const uniqueAutoAbsent = new Set(autoAbsences.map(a => a.employee_id)).size;
    
    return { disponibles, ausentes, total, uniqueFormalAbsent, uniqueAutoAbsent };
  }, [employees, activeAbsencesConsolidated, sourceContext, isAutoGenerated]);
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-900 dark:text-red-100">
                  {activeAbsencesConsolidated.filter(a => !isAutoGenerated(a)).length}
                </p>
                <p className="text-[10px] text-red-600 mt-0.5">
                  + {activeAbsencesConsolidated.filter(a => isAutoGenerated(a)).length} auto-generadas ⚠️
                </p>
              </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                onClick={async () => {
                  if (!confirm('¿Limpiar ausencias auto-generadas duplicadas? Solo se cancelarán duplicados, manteniendo siempre el registro más antiguo.')) return;
                  try {
                    const res = await base44.functions.invoke('cleanDuplicateAbsences', { dry_run: false });
                    const data = res.data;
                    toast.success(`Limpieza completada: ${data.cancelados} duplicados cancelados de ${data.duplicados_encontrados} encontrados.`);
                    queryClient.invalidateQueries({ queryKey: ['absences'] });
                  } catch (e) {
                    toast.error('Error en limpieza: ' + e.message);
                  }
                }}
                title="Limpiar ausencias duplicadas del sistema automático"
              >
            ...
            <Table>
              ...
              <TableBody>
                {filteredAbsences.map(abs => {
                  ...
                  const auto = isAutoGenerated(abs);
                  return (
                    <TableRow key={abs.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800 ${auto ? 'bg-amber-50/40 dark:bg-amber-950/20' : ''}`}>
                      <TableCell className="font-semibold">
                        <div className="flex items-center gap-1.5">
                          {auto && <Bot className="w-3.5 h-3.5 text-amber-500 flex-shrink-0" title="Generada automáticamente por el sistema" />}
                          {getEmployeeName(abs.employee_id)}
                        </div>
                        <div className="text-[10px] font-normal text-slate-400 mt-0.5">
                          {auto ? '⚠️ Auto-generada — requiere revisión RRHH' : `Por: ${abs.created_by || 'RRHH'}`}
                        </div>
                      </TableCell>
                      <TableCell>
                        <Badge variant="outline" className={auto ? 'border-amber-400 text-amber-700' : ''}>
                          {abs.tipo || '—'}
                        </Badge>
                      </TableCell>
                      <TableCell className="max-w-[180px]">
                        <span className="text-sm">{abs.motivo}</span>
                      </TableCell>
                      ...
                      <TableCell>
                        {auto ? (
                          <Badge className="bg-amber-500 text-white text-[10px]">Pendiente revisión</Badge>
