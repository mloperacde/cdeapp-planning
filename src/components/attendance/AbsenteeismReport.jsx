...
import {
  RefreshCw, Download, Users, AlertCircle, UserX, Clock, CalendarRange, ChevronDown, ChevronUp, LayoutList, Table2, Factory, Building2, BarChart3
} from "lucide-react";
    ...
    <div className="space-y-4">
      ...
      {processedReport && (
        <>
          ...
          {/* Análisis por grupos de departamento */}
          {processedReport.deptList.length > 0 && (() => {
            const prodStats = processedReport.calcGroupStats(processedReport.produccionDepts);
            const otherStats = processedReport.calcGroupStats(processedReport.otherDepts);
            const allStats = processedReport.calcGroupStats(processedReport.deptList);

            const StatGroup = ({ label, icon: Icon, iconColor, stats, depts }) => (
              <Card>
                ...
                <CardContent className="pt-0 space-y-3">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {[
                      { label: "Empleados", value: stats.empCount, cls: "text-slate-700" },
                      { label: "Ausencias", value: stats.abs, cls: "text-red-600" },
                      { label: "Inc. jornada", value: stats.inc, cls: "text-orange-600" },
                      { label: "% Ausencia", value: `${stats.pct}%`, cls: parseFloat(stats.pct) >= 10 ? "text-red-600" : parseFloat(stats.pct) >= 5 ? "text-orange-500" : "text-green-600" },
                    ].map(item => (
                  ...
                  {/* Tabla de departamentos del grupo */}
                  {depts.length > 0 && (
                    <div className="overflow-x-auto rounded border border-slate-100">
                      <table className="w-full text-xs">
                        <thead className="bg-slate-50 dark:bg-slate-800">
                          <tr>
                            <th className="text-left px-3 py-1.5 font-medium text-slate-600">Departamento</th>
                            <th className="text-center px-3 py-1.5 font-medium text-slate-600">Empl.</th>
                            <th className="text-center px-3 py-1.5 font-medium text-slate-600">Ausencias</th>
                            <th className="text-center px-3 py-1.5 font-medium text-slate-600">Inc.</th>
                            <th className="text-center px-3 py-1.5 font-medium text-slate-600">% Aus.</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                          {depts.map(d => {
                            const poss = d.employeeCount * processedReport.totalDays;
                            const pct = poss > 0 ? ((d.absences / poss) * 100).toFixed(1) : "0.0";
                            return (
                              <tr key={d.name} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                <td className="px-3 py-1.5 font-medium truncate max-w-[160px]">{d.name}</td>
                                <td className="px-3 py-1.5 text-center text-slate-500">{d.employeeCount}</td>
                                <td className="px-3 py-1.5 text-center">
                                  {d.absences > 0 ? <span className="text-red-600 font-semibold">{d.absences}</span> : <span className="text-slate-300">0</span>}
                                </td>
                                <td className="px-3 py-1.5 text-center">
                                  {d.incompletes > 0 ? <span className="text-orange-600 font-semibold">{d.incompletes}</span> : <span className="text-slate-300">0</span>}
                                </td>
                                <td className="px-3 py-1.5 text-center">
                                  <span className={`font-bold ${parseFloat(pct) >= 10 ? "text-red-600" : parseFloat(pct) >= 5 ? "text-orange-500" : "text-green-600"}`}>{pct}%</span>
                                </td>
                              </tr>
                            );
                          })}
            ...
            return (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                  <StatGroup label="General" icon={BarChart3} iconColor="text-indigo-500" stats={allStats} depts={processedReport.deptList} />
                  <StatGroup label="Producción" icon={Factory} iconColor="text-blue-600" stats={prodStats} depts={processedReport.produccionDepts} />
                  <StatGroup label="Otros departamentos" icon={Building2} iconColor="text-slate-500" stats={otherStats} depts={processedReport.otherDepts} />
                </div>
              </div>
            );
          })()}
