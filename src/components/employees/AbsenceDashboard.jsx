...
export default function AbsenceDashboard({ absences: propsAbsences, employees: propsEmployees }) {
  ...
  // Informe backend (DailyPresence como fuente de verdad) — coherente con AbsenceReportGenerator
  const { data: reportData, isLoading: reportLoading } = useQuery({
    queryKey: ['absenceReport'],
    queryFn: async () => {
  ...
  const byType = useMemo(() => {
    ...
    for (const emp of emps) {
      ...
      for (const [tipo, days] of Object.entries(emp.typeCounts)) {
        typeAgg[tipo] = (typeAgg[tipo] || 0) + days;
      }
    }
    return Object.entries(typeAgg).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value);
  }, [reportData]);
  ...
  const byEmployee = useMemo(() => {
    ...
    return emps
      ...
      .slice(0, 10)
      .map(e => ({ name: e.nombre, value: e.days12 }));
  }, [reportData]);
  ...
  const byDepartment = useMemo(() => {
    ...
    for (const emp of emps) {
      ...
      deptAgg[dept] = (deptAgg[dept] || 0) + (emp.days12 || 0);
    }
    return Object.entries(deptAgg)
      .map(([department, totalDays]) => ({ department, value: totalDays }))
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Días de ausencia por tipo (últimos 12 meses, según fichajes reales).
            </p>
          </CardHeader>
          <CardContent>
        ...
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Días laborables de ausencia por departamento (últimos 12 meses).
            </p>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={260}>
        ...
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Empleados con más días de ausencia (últimos 12 meses).
            </p>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={260}>
      ...
      <div className="grid grid-cols-1">
        <LongAbsenceAlert employees={employees} absences={absences} reportEmployees={reportData?.employees || []} />
      </div>
